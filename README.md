...
- `menu_simple_template.csv`
- `ingredients_simple_template.csv`
- `recipes_simple_template.csv`
- `menu_template.csv`
...
Simple import — основной формат для операционного заполнения. Он не требует `product_id`, `tax_rate`, `delivery_available`, gross/net weights и других технических колонок.

1. Сначала импортируйте меню через `menu_simple_template.csv`.
2. Затем импортируйте ингредиенты через `ingredients_simple_template.csv`.
3. После этого импортируйте рецептуры через `recipes_simple_template.csv`.

`menu_simple_template.csv`:

```csv
sku_name,category,sale_price,description,image_url,product_url,is_active
Бургер Рокки,Бургеры,450,"Булочка, котлета, сыр, соус, овощи",,,true
```

Обязательные поля: `sku_name`, `category`, `sale_price`. Пользователь не указывает `id`. Если SKU уже есть по паре `sku_name + category`, импорт обновит цену, описание, ссылки и `is_active`, но не создаст дубль.

`ingredients_simple_template.csv`:

```csv
ingredient_name,category,supplier,purchase_price,purchase_unit,comment
Говядина,Мясо,,850,kg,
Булочка,Хлеб,,25,piece,
```

Обязательные поля: `ingredient_name`, `purchase_price`, `purchase_unit`. Поддерживаемые единицы закупки: `kg`, `g`, `liter`, `ml`, `piece`. Если ингредиент уже есть по `ingredient_name`, импорт обновит цену, единицу, категорию, поставщика и комментарий без дубля. `edible_yield_percent` автоматически считается 100, `storage_loss_percent` — 0.

`recipes_simple_template.csv`:

```csv
sku_name,ingredient_name,quantity,unit,purchase_price,purchase_unit,cost_in_portion,comment
Бургер Рокки,Говядина,120,g,850,kg,,Цена за кг
Бургер Рокки,Булочка,1,piece,25,piece,,Цена за штуку
```

Обязательные поля: `sku_name`, `ingredient_name`, `quantity`, `unit`. `purchase_price` и `purchase_unit` можно не заполнять, если ингредиент уже есть в справочнике. Если ингредиента нет, нужно указать закупочную цену и единицу, иначе строка вернет ошибку.

`cost_in_portion` можно оставить пустым. Тогда себестоимость порции считается автоматически:

```text
kg -> g: purchase_price / 1000 * quantity
g -> g: purchase_price * quantity
liter -> ml: purchase_price / 1000 * quantity
ml -> ml: purchase_price * quantity
piece -> piece: purchase_price * quantity
```

Если `cost_in_portion` заполнен, он используется как ручная себестоимость ингредиента в порции, а строка помечается как `USER_PORTION_COST`. Если единицы несовместимы, например `kg -> piece`, импорт вернет ошибку по номеру строки.

В карточке SKU и таблице `/menu` после импорта видны ingredient cost, packaging, total variable, contribution и EBITDA/item. Для рецептурной себестоимости показывается режим `calculated`, `manual` или `mixed`.
